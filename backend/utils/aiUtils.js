...
9. Formát odpovědi: VŽDY odpověz pomocí XML-like tagů: <description>...</description>, <npc name="...">...</npc> (pro každou mluvící NPC), <action>...</action> (pokud se něco stane), <mechanics>...</mechanics> (pro hody kostkou hráče nebo jiné mechaniky), <options>...</options> (návrhy možných akcí).`;
  ...
  // Přidání shrnutí paměti, pokud existuje
  let memorySummaryText = '';
  if (gameState.memory_summary) {
    const memory = gameState.memory_summary;

    // Formátování NPC interakcí
    let npcInteractionsText = '';
    if (memory.npc_interactions && memory.npc_interactions.length > 0) {
      npcInteractionsText = memory.npc_interactions.map(npc =>
        `- ${npc.npc}: ${npc.relationship}. ${npc.key_events}`
      ).join('\n');
    }

    memorySummaryText = `
--- PAMĚŤ PŘÍBĚHU ---
Důležitá rozhodnutí: ${memory.character_decisions || 'Žádná'}
Sliby a závazky: ${memory.promises_made || 'Žádné'}
Důležitá místa: ${memory.important_locations || 'Žádná'}
Odhalená tajemství: ${memory.secrets_revealed || 'Žádná'}
${npcInteractionsText ? `\nInterakce s NPC:\n${npcInteractionsText}` : ''}
--- KONEC PAMĚTI ---
`;
  }
